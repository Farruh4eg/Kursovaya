...
$totalSearchResults = "SELECT COUNT(*) as total FROM books WHERE (book_genres LIKE '%adventures%') OR (book_genres LIKE '%приключения%')";
...
$sqlSearch = "SELECT * FROM books WHERE (book_genres LIKE '%adventures%') OR (book_genres LIKE '%приключения%') LIMIT $offset,$resultsPerPage";
...
<nav class='navBar'>
        ...
        <ul class='dropdown'>
            <li><a href='./list.php' class='genres' class='navMenu'>Жанры ▼</a>
                <div class='dropdown-content'>
                    <a href='./textbook.php'>Учебники</a>
                    <a href='./horror.php'>Ужасы</a>
                    <a href='./fantasy.php'>Фантастика</a>
                    <a href='./detective.php'>Детектив</a>
                    <a href='./adventures.php'>Приключения</a>
                    <a href='./novel.php'>Роман</a>
                </div>
            </li>
        </ul>
        <ul class='rightNav'>
...
<div id='responsiveNav' style='display: flex; flex-direction: column; position: fixed; top: 0; bottom: 0; right: -280px; z-index: 30; background-color: rgb(23, 26, 33); width: 280px; color: white; line-height: 2.5em; padding: 0 12px; transition: right 0.5s, left 0.5s; font-family: Nunito Sans, sans-serif'>
    <a href='./list.php' style='border-top: 1px solid #2f3138; border-bottom: 1px solid black'>Жанры</a>
